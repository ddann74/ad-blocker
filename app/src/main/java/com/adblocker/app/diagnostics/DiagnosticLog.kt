...
/**
 * A more verbose, technical companion to each module's user-facing Activity feed -
 ...
 * exception caught elsewhere in the app. Shared by all three modules (TikTok,
 * Spotify, Podcast) into one file, tagged per entry (e.g. "TIKTOK/AD", "PODCAST/SKIP")
 * so it stays useful for tuning or reporting a bug in any one of them.
 *
 * A no-op unless [isEnabled] returns true (backed by GlobalSettings, default off), so
 * callers never need to guard calls with their own enabled-check. Written to a plain
 * file in app-private storage rather than SharedPreferences, since entries can include
 * full raw text dumps that don't belong in a preferences store; size-capped so it
 * can't grow unbounded while logging is left on.
 */
class DiagnosticLog(context: Context, private val isEnabled: () -> Boolean) {
