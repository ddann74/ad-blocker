...
class SettingsRepository(context: Context) {
    ...
    /** Off by default. This never skips or blocks anything - browsing stays completely
      * normal. Instead, whenever the current video's own on-screen text mentions a
      * configured [subjectKeywords] entry, the app auto-taps TikTok's own Like button on
      * it (see [likeOptionKeywords] / TikTokActionCoordinator.attemptLikeCurrentVideo) as
      * a positive engagement signal, on the theory that TikTok's own recommendation
      * algorithm weighs likes/watch-time and will drift toward showing more of what you
      * consistently engage with. HONEST LIMIT: this can only nudge documented signals,
      * not verify or guarantee any actual shift in what gets recommended - that's
      * entirely up to TikTok's own black-box algorithm. Stays inert even if enabled
      * before any subject is configured. */
    ...
    companion object {
        private const val PREFS_NAME = "adblocker_tiktok_settings"
